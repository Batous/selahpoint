...
function showVerse(ref, text, version, path) {
  ...
  publishPresentationState({ type: 'verse', reference: ref, text, version, path });
}

function publishPresentationState(payload) {
  const state = { ...payload, updatedAt: new Date().toISOString() };
  try { localStorage.setItem('sangocast_presentation_state', JSON.stringify(state)); } catch(e) {}
  try {
    const channel = new BroadcastChannel('sangocast-presentation');
