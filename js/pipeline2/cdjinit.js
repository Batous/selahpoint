...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const controlBibleBtn = document.getElementById('btn-control-bible-keyboard');
  if (controlBibleBtn) {
    controlBibleBtn.addEventListener('click', () => {
      showBibleKeyboard();
    });
  }
...
if (lookupBtn) lookupBtn.addEventListener('click', async () => {
  ...
  if (verseCard) {
    verseCard.addEventListener('click', () => {
      ...
      const sessionActive = currentAudio || slideRanges.length > 0 || (stopBtn && stopBtn.style.display !== 'none');
      if (bar && sessionActive) bar.classList.add('active');
    });
  }
